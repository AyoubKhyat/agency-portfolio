...
export default function ProfilePage() {
  ...
  const cards = [
    { label: "Assigned to me", value: myStat?.assigned ?? 0, color: "text-gray-900" },
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-8 mb-6">
        <div className="flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center text-xl font-semibold text-white shadow-lg shadow-violet-100">
            {me.avatarInitials}
          </div>
          <div>
            <h1 className="text-xl font-semibold text-gray-900">{me.fullName}</h1>
            <div className="flex items-center gap-3 mt-1">
              <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-semibold uppercase ${ROLE_COLORS[me.role] ?? ""}`}>{me.role}</span>
              <span className="text-[13px] text-gray-400">{me.email}</span>
            </div>
          </div>
          <div className="ml-auto text-right">
            <div className="flex gap-4">
              <div>
                <p className="text-xl font-semibold text-gray-900">{myStat?.replyRate ?? 0}%</p>
                <p className="text-[10px] text-gray-400 uppercase">Reply rate</p>
              </div>
              <div>
                <p className="text-xl font-semibold text-gray-900">{myStat?.conversionRate ?? 0}%</p>
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {cards.map((c) => (
          <div key={c.label} className="bg-white rounded-2xl border border-gray-100 p-4 text-center">
            ...
            <p className="text-[10px] text-gray-400 mt-1 uppercase tracking-wide">{c.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <h2 className="text-[15px] font-semibold text-gray-900 mb-5">My Activity</h2>
        {activities.length === 0 ? (
          <p className="text-[13px] text-gray-400 py-8 text-center">No activity yet. Start prospecting!</p>
        ) : (
